...
Expected: 201
Received: 401
    at C:\Basavaraj\PlaywrightAutomationRS\tests\APITest.spec.js:27:31
```
...
> 27 |     expect(response.status()).toBe(201);
     |                               ^ Error: expect(received).toBe(expected) // Object.is equality
  28 |     expect(responseBody.name).toBe('Kumar');
  29 |     expect(responseBody.job).toBe('trainer');
  30 |   });
  31 |
  32 | test('Update user', async ({request})=>{
  33 |
  34 |
  35 | })
  36 |
  37 |
  38 | test('Delete user', async ({request})=>{
  39 |
  40 |     
  41 | })
```